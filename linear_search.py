...
print("Welcome to the Searching AI Track Robot"
      "\nThis bot will run in three different modes to find a stationary object in a 10x10 grid"
      "\n\n Modes: "
      "\n-Linear Search(Increases the position by one place)"
      "\n-Binary Search(Uses a binary equation)"
      "\n-Random Search(Random searches grids for the object)")
...
# Function that goes down the y axis a specific amount of times
def y_down():
    times = 10
    for i in range(times):
        down()

# Function that searches for the goal in a linear search method
def linear_search():
    y_up()
    right()
    y_down()
